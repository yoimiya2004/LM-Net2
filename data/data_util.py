...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    input_paths = list(scandir(input_folder))  # usually returns file names (not full path)
    ...
    for gt_path in gt_paths:
        gt_basename, _ = osp.splitext(osp.basename(gt_path))

        matched_lqs = []
        for ip in input_paths:
            ip_basename, _ = osp.splitext(osp.basename(ip))
            # 精确一点：要求 "0025_" 前缀，避免 "002" 匹配到 "0025"
            if ip_basename.startswith(gt_basename + "_") or ip_basename == gt_basename:
                matched_lqs.append(ip)

        if len(matched_lqs) == 0:
            print(f"[Warning] No LQ found for GT {gt_basename}")
            continue

        for input_path in matched_lqs:
            input_full_path = osp.join(input_folder, input_path)
    ...
    print(f"Total paired samples: {len(paths)}")
    return paths
